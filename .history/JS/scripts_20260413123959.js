...
const PROJECTS = [
  {
    ...
    githubUrl: '#',   // ← ضعي رابط GitHub هنا عند الجاهزية
    ar: {
      title: 'تطبيق فزعه',
 ...
 *
 * Each card shows two action buttons:
 *   1. "التفاصيل / Details" → navigates to a dedicated
 *      project page: projects/project-{id}.html
 *   2. "GitHub"             → external link (set per project
 *      via the githubUrl field, or "#" if not set yet)
 *
 * Clicking anywhere else on the card body is disabled
 * to avoid accidental navigation conflicts.
 * ─────────────────────────────────────────────
...
function renderProjects(filter = 'all') {
  ...
  grid.innerHTML = list.map((project) => {
    ...
    /* Path to this project's dedicated details page */
    const detailsUrl = `projects/project-${project.id}.html`;

    /* GitHub URL — use project.githubUrl if defined, else "#" */
